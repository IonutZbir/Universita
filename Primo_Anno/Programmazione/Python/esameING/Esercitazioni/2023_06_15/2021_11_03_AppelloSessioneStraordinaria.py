...
def compito(comando,input):
    if comando == "trova bersaglio":
        ...
        output = determina_se_non_dice_la_verità(input)
    return output
...
def codifica_messaggio(mess: str):
    cod_mess = ""
    for i in range(len(mess)):
        if mess[i] == "z":
            ...
            cod_mess += "A"
        elif (ord("A") <= ord(mess[i]) <= ord("Y")) or (ord("a") <= ord(mess[i]) <= ord("y")):
            cod_mess += chr(ord(mess[i]) + 1)
        else:
            cod_mess += mess[i]
    return cod_mess    
...
def analizza_accessi_telefonici(file):
    FILE = open(file, "r")
    d = {}
    for row in FILE.readlines():
        n1, n2, orario = row.split(" ")
        d[n1] = d.get(n1, [])
        d[n1].append(n2)
    mx = (0, 0)
    for k in d:
...
def determina_se_non_dice_la_verità(mess):
    voc_mess = {}
    voc = ["a", "e", "i", "o", "u"]
    for x in mess:
        if x in voc:
            voc_mess[x] = voc_mess.get(x, 0) + 1
    tot = sum(voc_mess.values())
    if tot % 2  == 0:
        return True
    return False
...
#       a.rimuovi("cane")
    def rimuovi(self, parola):
        self.mess = self.mess.replace(" " + parola, "", 1)
...
# che deve essere sostituita con la a
    def codifica(self):
        return codifica_messaggio(self.mess)
