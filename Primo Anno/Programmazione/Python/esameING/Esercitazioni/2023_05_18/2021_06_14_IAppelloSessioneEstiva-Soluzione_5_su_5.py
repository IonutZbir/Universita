...
def terzoEsercizio(DNA_Virus, SottosequenzeDaCercare):
    ...
    # Il ciclo for sottostante i cui viene usato il
    # metodo count() non è corretto.
    ...
    # ------------------------
    # ERRORE
    # [('GTA', 1), ('AA', 3), ('GT', 2), ('AAGGAG', 0)]
    # non sembra essere giusto rispetto all'input
    # GGGATAGGTAAGAAAAGT
    # ['GTA', 'AA', 'GT', 'AAGGAG']
    # dovrebbe invece essere così:
    # [('GTA', 1), ('AA', 4), ('GT', 2), ('AAGGAG', 0)]

    # Il problema (come si può intuire dal test con input 'AA')
    # è che count considera match non sovrapposti (infatti inizia
    # a cercare, e quindi a contare, un nuovo match a partire
    # dalla fine del match appena trovato)

    # for seq in SottosequenzeDaCercare:
    #     occorrenze[seq] = DNA_Virus.count(seq)

    # non conviene "scambiare" i due cicli, perché altrimenti
    # per ogni "sottosequenza da cercare" andremmo a ripetere lo
    # slicing per le medesime posizioni, cosa non efficiente

    for i in range(len(DNA_Virus)):
        # in alternativa allo slicing si sarebbe potuto
        # usare "i" come secondo argomento dell'invocazione
        # del metodo "startswith"
        suffisso_dna = DNA_Virus[i:]
        for seq in SottosequenzeDaCercare:
            if suffisso_dna.startswith(seq):
                if seq in occorrenze:
                    occorrenze[seq] += 1  # equivalente a occorrenze[seq] = occorrenze[seq] + 1
                else:
                    occorrenze[seq] = 1
    ...
    # for seq in SottosequenzeDaCercare:
    #     if seq in occorrenze:
    #         coppia = (seq, occorrenze[seq])
    #     else:
    #         coppia = (seq, 0)
    #     lista_occorrenze.append(coppia)

    for seq in SottosequenzeDaCercare:
        if seq in occorrenze:
            lista_occorrenze.append((seq, occorrenze[seq]))
        else:
            lista_occorrenze.append((seq, 0))
...
def quartoEsercizio(s1: str, s2: str):
    sottosequenze1 = genera_sottosequenze(s1)
    sottosequenze2 = genera_sottosequenze(s2)

    sottosequenze_comuni = sottosequenze1.intersection(sottosequenze2)

    lunghezza_massima = 0
    for seq in sottosequenze_comuni:
        if lunghezza_massima < len(seq):
            lunghezza_massima = len(seq)

    return lunghezza_massima


def genera_sottosequenze(dna):
    sottosequenze = set()
    for i in range(len(dna)):
        for j in range(i + 1, len(dna) + 1):
            sottosequenze.add(dna[i:j])
...
def main():
    ...
    test_terzo_esercizio = [("GATTAGA", ["GA", "TA", "ATTA"], [("GA", 2), ("TA", 1), ("ATTA", 1)]),
                            ("GGGATAGGTAAGAAAAGT", ["GTA", "AA", "GT", "AAGGAG"],
                             [("GTA", 1), ("AA", 4), ("GT", 2), ("AAGGAG", 0)])]
