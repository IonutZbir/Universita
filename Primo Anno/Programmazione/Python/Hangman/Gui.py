...
class HangmanGame:
    def __init__(self):
        ...
        Label(self.input_frame, text='Used Characters', fg='black', font=('Arial', 8)).grid(column=0, row=0, padx=5)

        Label(self.input_frame, text='Make a Guess..', fg='red', font=('Arial', 12, 'bold')).grid(column=0, row=1, columnspan=3)

        self.buttons_per_row = 10
        for i, char in enumerate(range(ord('a'), ord('z') + 1)):
            button = self.create_button(chr(char))
            self.buttons.add(button)
            button.grid(row=i // self.buttons_per_row, column=i % self.buttons_per_row, padx=5, pady=5)
        ...
        self.restart_button.grid(
            row=i // self.buttons_per_row, column=(i % self.buttons_per_row) + 1, columnspan=5, padx=5)
