...
int binarySearchLastOccurence(int* arr, int n, int start, int end){
	...
	if (arr[m] == n){
		...
		if (nextInstance == -1)  return m; // Questo è l'ultimo elemento uguale a n
		else return nextInstance; // Restituisci l'indice dell'ultima istanza trovata nella parte destra
	}
	if (arr[m] > n) return binarySearchLastOccurence(arr, n, start, m - 1);
	else return binarySearchLastOccurence(arr, n, m + 1, end);
}
...
int binarySearchFirstOccurence(int* arr, int n, int start, int end){
	...
	int m = (start + end) / 2;
	if (arr[m] == n) return m;
	if (arr[m] > n) return binarySearchFirstOccurence(arr, n, start, m - 1);
	else return binarySearchFirstOccurence(arr, n, m + 1, end);
}
