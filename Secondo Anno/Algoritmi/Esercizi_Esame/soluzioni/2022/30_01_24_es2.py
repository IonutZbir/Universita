...
def ContaSumAnt(graph, current_node, sum_ant, delta, visited=None):
    ...
    print(f'Node {current_node} - Value: {val} - Sum: {sum_ant}')

    if sum_ant >= delta:
        return sum([ContaSumAnt(graph, neighbor, sum_ant, delta, visited) for neighbor in neighbors]) + 1
    else:
        return sum([ContaSumAnt(graph, neighbor, sum_ant, delta, visited) for neighbor in neighbors])
...
root_node = 1
result = ContaSumAnt(albero_binario, root_node, 0, 20)
print("Risultato dell'algoritmo ContaSumAnt:", result)
