...
def BluGiallo(graph, current_node, b = False, visited=None):
    ...
    if(col == 'G' and b == False):
        b = True

    neighbors = list(graph.neighbors(current_node))

    print(f'Node {current_node} - Value: {val} - Color: {col} - Bool: {b}')

    return sum([BluGiallo(graph, neighbor, b, visited) for neighbor in neighbors]) + 1
...
root_node = 1
result = BluGiallo(albero_binario, root_node)
