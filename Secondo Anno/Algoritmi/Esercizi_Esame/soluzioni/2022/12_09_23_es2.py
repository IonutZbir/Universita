...
def ContaProf(graph, current_node, h1, h, h2, visited=None):
    ...
    print(f'Node {current_node} - Value: {val} - H1: {h1} - H: {h} - H2: {h2}')

    if  h1 <= h <= h2:
        return sum([ContaProf(graph, neighbor, h1, h + 1, h2, visited) for neighbor in neighbors]) + 1
    else:
        return sum([ContaProf(graph, neighbor, h1, h + 1, h2, visited) for neighbor in neighbors])
...
root_node = 1
result = ContaProf(albero_binario, root_node, 1, 0, 3)
