...
void *producer(void *arg) {
  while (TRUE) {
    down(&empty); // Decrementa empty, quindi aggiungo un elemento nel buffer.
                  // Se empty == 0, il buffer è pieno quindi il produttore si
                  // blocca (si mette in attesa che il consumatore consuma
                  // almeno un elemento)
    ...
    // Il buffer potrebbe essere gia stato modificato dal consumatore poichè si
    // trova al di fuori della regione critica
    ...
    // thread si prende la zona critica e se empty == 0 allora si blocca nella
    // regione critica, bloccando tutta l'esecuzione del programma
  }
}
...
void *consumer(void *arg) {
  while (TRUE) {
    down(&full); // Decrementa full, quindi consumo un elemento dal buffer, se
                 // full == 0, il buffer è vuoto quindi il consumatore si blocca
                 // (si mette in attesa che il produttore produca almeno un
                 // elemento)
    down(&mutex); // Prova a entrare nella regione critica, se il mutex è 0
                  // attende altrimenti entra nella regione critica
    ...
    up(&mutex); // Esce dalla regione critica
    up(&empty); // Segnala che c'è un posto vuoto in più nel buffer, incrementa
                // empty
  }
}
